...
def replace_keys(shp_config, keys):
    for option in shp_config:
        if not isinstance(shp_config[option], str):
            continue
...
def sql_url(user, password, url, port, db):
     return "postgresql://{user}:{password}@{url}:{port}/{db}".format(user=user, password=password, url=url, port=port, db=db)

def get_safe_engine(user, password, url, port, db):
    engine = sqlalchemy.create_engine(sql_url(user, password, url, port, db))
    try:
...
def get_config(ifolder):
    ...
    for config_file in glob.glob(os.path.join(ifolder, "**/dbs.json"), recursive=True):
        with open(config_file) as json_file:
            data = json.load(json_file)
        for db in data:
            if 'port' not in data[db]:
                data[db]['port'] = 5432
            if 'password' not in data[db]:
                engine = False
                while not engine:
                    password = input()
                    engine = get_safe_engine(data[db]['user'], password, data[db]['url'], data[db]['port'], data[db]['db'])
            else:
                engine = get_safe_engine(data[db]['user'], data[db]['password'], data[db]['url'], data[db]['port'], data[db]['db'])
                if not engine:
...
def shp2postgis(shp_file_path):
    ...
    if debug:
        print("shape config {}".format(shp_file_path))
        print(shp_config)
